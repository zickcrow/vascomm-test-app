<template>
    <div class="relative flex justify-center min-h-screen bg-gray-100 items-top dark:bg-gray-900 sm:items-center sm:pt-0">
        <div v-if="canLogin" class="fixed top-0 right-0 hidden px-6 py-4 sm:block">
            <inertia-link v-if="$page.props.user" :href="dashboardRoute" class="text-sm text-gray-700 underline">
                Dashboard
            </inertia-link>
...
<script>
    export default {
        ...
        computed: {
            dashboardRoute() {
                return this.$page.props.user.is_admin ? route('admin.dashboard') : route('dashboard')
            },
        }
    }
</script>
